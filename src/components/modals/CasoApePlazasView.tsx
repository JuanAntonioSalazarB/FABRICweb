...
export default function CasoApePlazasView() {
  ...
  const handleWaitlistSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setWlSuccess(true);
        window.location.href = `/aplicar?wl-name=${encodeURIComponent(wlName)}&wl-company=${encodeURIComponent(wlCompany)}&wl-scenario=rescue`;
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <span className="badge-premium mb-2 inline-block">REAL CASE STUDY · MIGRATION</span>
        <h2 className="text-2xl font-serif text-white font-light">Caso APE Plazas: Estabilización de Facturación Masiva</h2>
        <p className="text-zinc-500 text-xs mt-1">Óptima facturación de arrendamientos comerciales y timbrado SAT masivo sin middlewares.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 text-xs font-mono">
        <div className="md:col-span-8 text-zinc-400 space-y-4 font-sans text-sm leading-relaxed">
          ...
          <p>
            FABRIC asumió el control del proyecto e inyectó conectores modularizados para el timbrado masivo SAT CFDI 4.0 directamente mediante bases de datos Oracle Fusion, bajando el tiempo total a solo 4 horas y con **cero incidencias críticas de severidad alta** durante el primer cierre contable de abril de 2026.
          </p>
        </div>
        ...
        <div className="md:col-span-4 space-y-4">
          ...
          <div className="border border-accent/20 p-5 bg-accent/5 space-y-3">
            ...
            <div className="space-y-2">
              <div>
                <span className="text-zinc-500 block text-[10px]">TIEMPO CICLO:</span>
                <span className="text-white font-bold">Reducido a 4 horas</span>
              </div>
              <div>
                <span className="text-zinc-500 block text-[10px]">CUMPLIMIENTO:</span>
                <span className="text-emerald-500 font-bold">✓ Cierre sin incidencias</span>
              </div>
              <div>
                <span className="text-zinc-500 block text-[10px]">TASA DE TIMBRADO:</span>
                <span className="text-white font-bold">99.98% primer intento</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 text-zinc-500">
            <div className="flex items-center gap-2"><Calendar className="w-3.5 h-3.5 text-accent" /> <span>06 de Abril 2026</span></div>
            <div className="flex items-center gap-2"><Database className="w-3.5 h-3.5 text-accent" /> <span>Oracle Fusion Cloud</span></div>
          </div>
        </div>
      </div>
